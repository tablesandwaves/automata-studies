...
export class ChordMelodySequencer {
  ...
  constructor(configFilepath) {
    ...
    this.#chordVoice         = new Voice(midiPort, 1);
    this.#accompanimentVoice = new Voice(midiPort, 2);
    this.#beatVoice          = new Voice(midiPort, 10);

    this.#voiceMessageChannel = new MessageChannel();
  ...
  run() {
    ...
    this.#voiceMessageChannel.port2.postMessage(1);
  }
  ...
  #setupChordsMessageReceiver() {
    this.#voiceMessageChannel.port1.on("message", (chordRoot) => {
      ...
      if (this.#iterationCount > this.#configuration.iterations) {
        setTimeout(() => {
          this.#voiceMessageChannel.port1.close();
          this.#voiceMessageChannel.port2.close();
        }, 3000);

        return;
      }

      const duration = this.#iterationCount < 5 || this.#iterationCount > this.#configuration.iterations - 4 ? 1500 : this.#randomDuration();
      const chord    = this.#randomChord(chordRoot);

      this.#beatVoice.playNote(this.#randomKick(), 100);
      chord.midi.forEach(note => this.#chordVoice.playNote(note, duration));
  ...
  #setupMelodyMessageReceiver() {
    this.#voiceMessageChannel.port2.on("message", (data) => {
      ...
      if (data.duration >= 500) {
        setTimeout(() => {
          this.#beatVoice.playNote(Math.random() > 0.5 ? this.#randomPerc() : this.#randomSnare(), 100);
        }, data.duration / 4);
      }
      ...
      if (note !== undefined)
        setTimeout(() => {
          ...
          this.#accompanimentVoice.playNote(note, data.duration);
        }, data.duration / 2);
      ...
      setTimeout(() => this.#voiceMessageChannel.port2.postMessage(this.#midiToScaleDegree(note)), data.duration);

      // Finally log the chord, accompaniment note and duration for the current cycle iteration.
      this.#logEvent(this.#iterationCount, data.chord, note, data.duration);
    });
  }
}
